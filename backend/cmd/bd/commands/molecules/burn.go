...
func runMolBurn(cmd *cobra.Command, args []string) {
	...
	if len(args) == 1 {
		burnSingleMolecule(ctx, cliCtx, args[0], dryRun, force)
		return
	}

	// Multiple IDs: batch mode for efficiency
	burnMultipleMolecules(ctx, cliCtx, args, dryRun, force)
}

// burnSingleMolecule handles the single molecule case (original behavior)
func burnSingleMolecule(ctx context.Context, cliCtx *cli.Context, moleculeID string, dryRun, force bool) {
	store := cliCtx.GetStore()
	...
	if rootIssue.Ephemeral {
		...
		burnWispMolecule(ctx, cliCtx, resolvedID, dryRun, force)
	} else {
		// Mol: cascade delete with tombstones
		burnPersistentMolecule(ctx, cliCtx, resolvedID, dryRun, force)
	}
}

// burnMultipleMolecules handles batch deletion of multiple molecules efficiently
func burnMultipleMolecules(ctx context.Context, cliCtx *cli.Context, moleculeIDs []string, dryRun, force bool) {
	store := cliCtx.GetStore()
	var wispIDs []string
	...
	if len(wispIDs) == 0 && len(persistentIDs) == 0 {
		if cliCtx.IsJSONOutput() {
			cliCtx.OutputJSON(BatchBurnResult{FailedCount: len(failedResolve)})
		} else {
	...
	for _, id := range persistentIDs {
		subgraph, err := template.LoadSubgraph(ctx, store, id)
		if err != nil {
			if !cliCtx.IsJSONOutput() {
		...
		// Delete issues directly
		sqliteStore, ok := store.(*sqlite.SQLiteStorage)
		if ok {
			for _, issueID := range issueIDs {
				_ = sqliteStore.DeleteIssue(ctx, issueID)
			}
		}
...
func burnWispMolecule(ctx context.Context, cliCtx *cli.Context, resolvedID string, dryRun, force bool) {
	store := cliCtx.GetStore()

	// Load the molecule subgraph
	subgraph, err := template.LoadSubgraph(ctx, store, resolvedID)
	if err != nil {
...
func burnPersistentMolecule(ctx context.Context, cliCtx *cli.Context, resolvedID string, dryRun, force bool) {
	store := cliCtx.GetStore()
	...
	fmt.Printf("%s Burned mol: %d issues deleted\n", ui.RenderPass("✓"), len(issueIDs))
...
// registerBurnCmd registers the burn command with the mol command
func registerBurnCmd() {
