  ...
  bd mol current <id> --range 100-150  # Show steps 100-150`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if len(args) == 1 {
			...
			if stats.Total > LargeMoleculeThreshold && !explicitSteps && !cliCtx.IsJSONOutput() {
			...
			molecules = findInProgressMolecules(ctx, store, daemonClient, agent)
			if len(molecules) == 0 {
				if cliCtx.IsJSONOutput() {
					cliCtx.OutputJSON([]interface{}{})
					return
				}
...
func GetMoleculeProgress(ctx context.Context, s storage.Storage, moleculeID string) (*MoleculeProgress, error) {
	subgraph, err := template.LoadSubgraph(ctx, s, moleculeID)
	if err != nil {
		return nil, err
	}
...
func findInProgressMolecules(ctx context.Context, s storage.Storage, daemonClient *rpc.Client, agent string) []*MoleculeProgress {
...
func sortStepsByDependencyOrder(steps []*StepStatus, subgraph *MoleculeSubgraph) {
...
// registerCurrentCmd registers the current command with the mol command
func registerCurrentCmd() {
