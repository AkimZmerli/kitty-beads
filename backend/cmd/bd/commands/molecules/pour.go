...
func runPour(cmd *cobra.Command, args []string) {
	...
	for _, attachArg := range attachFlags {
		...
		if !IsProtoIssue(attachIssue) {
			fmt.Fprintf(os.Stderr, "Error: %s is not a proto (missing '%s' label)\n", attachID, MoleculeLabel)
			os.Exit(1)
		}
		attachSubgraph, err := template.LoadSubgraph(ctx, store, attachID)
		if err != nil {
	...
	for _, attach := range attachments {
		attachVars := template.ExtractRequiredVariables(attach.subgraph)
	...
	if cliCtx.IsJSONOutput() {
		...
		cliCtx.OutputJSON(pourResult{result, totalAttached, "liquid"})
		return
	}
...
// registerPourCmd registers the pour command with the mol command
func registerPourCmd() {
