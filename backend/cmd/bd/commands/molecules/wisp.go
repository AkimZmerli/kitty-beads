...
func runWispCreate(cmd *cobra.Command, args []string) {
	cliCtx := cli.Get()
	cliCtx.CheckReadonly("wisp create")
	...
	if subgraph == nil {
		...
		if !strings.HasPrefix(protoID, "bd-") && !strings.HasPrefix(protoID, "gt-") && !strings.HasPrefix(protoID, "mol-") {
			...
			if resolved, err := resolvePartialIDDirect(ctx, store, protoID); err == nil {
				protoID = resolved
			}
		}
		...
		subgraph, err = template.LoadSubgraph(ctx, store, protoID)
		if err != nil {
	...
	vars = template.ApplyVariableDefaults(vars, subgraph)

	// Check for missing required variables (those without defaults)
	requiredVars := template.ExtractRequiredVariables(subgraph)
	...
	if dryRun {
		...
		for _, issue := range subgraph.Issues {
			newTitle := template.SubstituteVariables(issue.Title, vars)
			fmt.Printf("  - %s (from %s)\n", newTitle, issue.ID)
		}
		return
	}
	...
	if cliCtx.IsJSONOutput() {
		type wispCreateResult struct {
			*template.InstantiateResult
			Phase string `json:"phase"`
		}
		cliCtx.OutputJSON(wispCreateResult{result, "vapor"})
		return
	}
...
func resolvePartialIDDirect(ctx context.Context, store interface{ GetIssue(context.Context, string) (*types.Issue, error); SearchIssues(context.Context, string, types.IssueFilter) ([]*types.Issue, error) }, partial string) (string, error) {
...
func runWispList(cmd *cobra.Command, args []string) {
	...
	ctx := cliCtx.GetRootCtx()
	store := cliCtx.GetStore()
	daemonClient := cliCtx.GetDaemonClient()
	...
	if store == nil && daemonClient == nil {
		if cliCtx.IsJSONOutput() {
			cliCtx.OutputJSON(WispListResult{
				Wisps: []WispListItem{},
				Count: 0,
			})
		} else {
...
func runWispGC(cmd *cobra.Command, args []string) {
	...
	if len(abandoned) == 0 {
		if cliCtx.IsJSONOutput() {
			cliCtx.OutputJSON(WispGCResult{
				CleanedIDs:   []string{},
				CleanedCount: 0,
				DryRun:       dryRun,
			})
...
// registerWispCmd registers the wisp command and its subcommands
func registerWispCmd() {
