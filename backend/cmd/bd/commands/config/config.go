// Package config implements the bd CLI config management commands.
package config
...
// ensureDirectMode switches from daemon to direct mode if needed.
// Returns an error if the switch fails.
func ensureDirectMode(reason string) error {
	cliCtx := cli.Get()
	if cliCtx.GetDaemonClient() != nil {
		if err := cliCtx.FallbackToDirectMode(reason); err != nil {
			return err
		}
	}
	return nil
...
var configSetCmd = &cobra.Command{
	...
	Run: func(_ *cobra.Command, args []string) {
		...
		if cliCtx.IsJSONOutput() {
			cliCtx.OutputJSON(map[string]string{
				"key":   key,
				"value": value,
			})
		} else {
...
// Register adds config commands to the root command.
