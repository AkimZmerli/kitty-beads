...
func TestConfigCommands(t *testing.T) {
	...
	if len(cfg) < 3 {
		t.Errorf("Expected at least 3 config entries, got %d", len(cfg))
	}

	if cfg["test.key"] != "updated-value" {
		t.Errorf("Expected 'updated-value' for test.key, got '%s'", cfg["test.key"])
	}
	if cfg["jira.url"] != "https://example.atlassian.net" {
		t.Errorf("Expected jira.url in config, got '%s'", cfg["jira.url"])
	}
	if cfg["jira.project"] != "PROJ" {
		t.Errorf("Expected jira.project in config, got '%s'", cfg["jira.project"])
	}
...
func TestConfigNamespaces(t *testing.T) {
	...
	for key, expected := range namespaces {
		if cfg[key] != expected {
			t.Errorf("Expected '%s' for %s in GetAllConfig, got '%s'", expected, key, cfg[key])
		}
	}
}
