...
func runMolSquash(cmd *cobra.Command, args []string) {
	cliCtx := cli.Get()
	cliCtx.CheckReadonly("mol squash")
	...
	if len(wispChildren) == 0 {
		if cliCtx.IsJSONOutput() {
			cliCtx.OutputJSON(SquashResult{
...
// registerSquashCmd registers the squash command with the mol command
func registerSquashCmd() {
