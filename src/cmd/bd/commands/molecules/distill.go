...
func runMolDistill(cmd *cobra.Command, args []string) {
	...
	subgraph, err := template.LoadSubgraph(ctx, store, epicID)
	if err != nil {
...
// GetFormulaSearchPaths returns the formula search paths.
...
func subgraphToFormula(subgraph *MoleculeSubgraph, name string, replacements map[string]string) *formula.Formula {
...
// countSteps counts the total number of steps in a formula
func countSteps(steps []*formula.Step) int {
	return len(steps)
}

// printFormulaStepsTree prints the formula steps as a tree
...
// registerDistillCmd registers the distill command with the mol command
func registerDistillCmd() {
