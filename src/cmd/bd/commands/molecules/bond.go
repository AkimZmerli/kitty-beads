...
func runMolBond(cmd *cobra.Command, args []string) {
	cliCtx := cli.Get()
	cliCtx.CheckReadonly("mol bond")

	ctx := cliCtx.GetRootCtx()
	store := cliCtx.GetStore()
	actor := cliCtx.GetActor()
	...
	if dryRun {
		...
		if issueA != nil {
			idA = issueA.ID
			aIsProto = IsProtoIssue(issueA)
		}
		if issueB != nil {
			idB = issueB.ID
			bIsProto = IsProtoIssue(issueB)
		}
		...
		if childRef != "" {
			resolvedRef := template.SubstituteVariables(childRef, vars)
...
// Exported for use by pour.go and wisp.go.
...
func operandType(isProtoFlag bool) string {
	if isProtoFlag {
		return "proto"
	}
	return "molecule"
...
func bondProtoMolWithSubgraph(ctx context.Context, s storage.Storage, protoSubgraph *MoleculeSubgraph, proto, mol *types.Issue, bondType string, vars map[string]string, childRef string, actorName string, ephemeralFlag, pourFlag bool) (*BondResult, error) {
	// Use provided subgraph or load from DB
	subgraph := protoSubgraph
	if subgraph == nil {
		var err error
		subgraph, err = template.LoadSubgraph(ctx, s, proto.ID)
		if err != nil {
	...
	requiredVars := template.ExtractAllVariables(subgraph)
...
func resolveOrCookToSubgraph(ctx context.Context, s storage.Storage, operand string, vars map[string]string) (*MoleculeSubgraph, bool, error) {
	// First, try to resolve as an existing issue
	id, err := utils.ResolvePartialID(ctx, s, operand)
	if err == nil {
		issue, err := s.GetIssue(ctx, id)
		if err == nil {
			...
			if IsProtoIssue(issue) {
				subgraph, err := template.LoadSubgraph(ctx, s, id)
				if err != nil {
...
// registerBondCmd registers the bond command with the mol command
func registerBondCmd() {
