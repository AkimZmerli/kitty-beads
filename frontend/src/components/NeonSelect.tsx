...
export function NeonSelect({ options, value, onChange, placeholder = 'Select...', className = '' }: NeonSelectProps) {
  ...
  return (
    <div ref={containerRef} className={`relative ${className}`}>
      ...
      <button
        ...
        `}
      >
        <span className={`whitespace-nowrap ${selectedOption ? 'text-text-primary' : 'text-text-muted'}`}>
      ...
      {isOpen && (
        ...
        >
          <div className="max-h-[300px] overflow-y-auto py-1">
            {options.length === 0 ? (
              <div className="px-4 py-3 text-text-muted text-sm">No options</div>
            ) : (
              options.map((option) => (
                <button
                  key={option.value}
